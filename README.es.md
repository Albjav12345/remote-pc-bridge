...
[![Read in English](https://img.shields.io/badge/Read%20in-English-2563eb?style=for-the-badge)](README.md)
...
![Imagen de presentación de Remote PC Bridge con datos ficticios](docs/media/remote-pc-bridge-cover.png)

La imagen utiliza datos ficticios y no establece conexiones reales. [Descargar el icono de la aplicación (PNG)](docs/media/remote-pc-bridge-icon.png).
...
![Datos de la instalación en el asistente](docs/images/setup.es.png)

![Reglas y flasheo del ESP32 desde la aplicación](docs/images/flash.es.png)

La [guía de instalación](docs/SETUP.es.md) desarrolla cada paso, incluido cómo localizar la MAC y qué comprobar si el puerto COM no aparece. La aplicación guía la creación de cuentas y el flasheo; la activación de Wake-on-LAN, la instalación de Sunshine/Tailscale/Moonlight y la publicación de reglas requieren acceso a sus respectivas interfaces.
...
Para regenerar la imagen de presentación a partir de la captura ficticia del repositorio, instala `Pillow` con pip y ejecuta `python scripts/build_presentation_image.py`. Ejecuta `./scripts/Make-Icon.ps1` en PowerShell para exportar el PNG de 512 px a partir del icono de la aplicación. Estos scripts no leen ajustes locales ni abren la aplicación.
